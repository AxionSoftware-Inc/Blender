...
This directory records architectural contracts, validated backend behavior, current numerical design, and future extension rules.
...
6. the subsystem-specific documents relevant to the task.

## Engine and domain architecture

### `DOMAIN_SYSTEM.md`

Core-vs-domain boundary, capability composition, renderer independence, semantic visualization rules, and long-term modularity target.
...
Numerical method/run metadata and provenance principles.
...
Renderer-neutral presentation intent: presets, camera, color scales, legends, axes, annotations, lighting, animation grammar, accessibility, and quality policy.

### `BLENDER_PREMIUM_PRESENTATION.md`

How Blender may realize premium presentation intent through native materials, lights, camera, attributes, Geometry Nodes, compositor, and incremental updates.
...
Canonical scientific scenes used as end-to-end integration and premium-quality targets.
...
Files such as native/GPU buffer design, premium presentation runtime phases, and external plugin entry-point discovery may describe architecture that is intentionally specified before implementation.
